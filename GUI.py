...
def select(case):
    ...
    match case:
        ...
        case "Change values":
            ...
            label4 = customtkinter.CTkLabel(master=frame4, text="Current values: Income: €" + str(Finance.income) + " / Savings: €" + str(Finance.savings) + " / Spendings: €" + str(Finance.spendings), text_color="pink", font=("Roboto", 16))
            ...
            def change_value():
                    ...
                    case "Income":
                        ...
                        if new_value is not None and new_value != "":
                            if string_to_num(new_value) is False or string_to_num(new_value) <= 0:
                                ...
                                Finance.update_income(new_value)
                                Finance.log.change_values("Income", new_value)
                    ...
                    case "Savings":
                        ...
                        if new_value is not None and new_value != "":
                            if string_to_num(new_value) is False or string_to_num(new_value) < 0:
                                ...
                                new_value = string_to_num(new_value)
                                Finance.update_savings(new_value, Finance.current_date)
                                Finance.log.change_values("Savings", new_value)
                    ...
                    case "Spendings":
                        ...
                        if new_value is not None and new_value != "":
                            if string_to_num(new_value) is False or string_to_num(new_value) < 0:
                                ...
                                new_value = string_to_num(new_value)
                                Finance.update_spendings(new_value)
                                Finance.log.change_values("Spendings", new_value)
                ...
                label6 = customtkinter.CTkLabel(master=frame6, text="Values updated: Income: €" + f"{Finance.income:.2f}" + " / Savings: €" + f"{Finance.savings:.2f}" + " / Spendings: €" + f"{Finance.spendings:.2f}", text_color="pink", font=("Roboto", 16))
        ...
        case "Projection calculations":
            ...
            label4 = customtkinter.CTkLabel(master=frame4, text=f"Income: €{Finance.income:.2f} / Savings: €{Finance.savings:.2f} / Spendings: €{Finance.spendings:.2f}", text_color="pink", font=("Roboto", 16))
            label4.grid(row=1, column=0, pady=10, padx=10)
            ...
            def calculate_projection():
                ...
                def save_projection(savings_result):
                    Finance.savings = savings_result
                    Finance.save_savings()
                    Finance.log.projection_calculation(f"{savings_result:.2f}")
            ...
            def interest_selection(choice):
                ...
                match choice:
                    case "Simple Interest":
                        ...
                        label5 = customtkinter.CTkLabel(master=frame4, text=f"Savings: €{Finance.savings:.2f}", text_color="pink", font=("Roboto", 16))
                        ...
                        def calculate_simple_interest():
                            ...
                            if interest_money is not None and interest_money != "":
                                if string_to_num(interest_money) is False or string_to_num(interest_money) <= 0 or string_to_num(interest_money) > Finance.savings:
                                    if frame6 is None or not frame6.winfo_exists():
                                        create_frame6(0, 2, frame3)
                                    error_label = customtkinter.CTkLabel(master=frame6, text="Invalid portion of savings affected by interest. Please enter a value between 0 and " + str(Finance.savings), text_color="pink", font=("Roboto", 16))
                                    error_label.grid(row=0, column=0, pady=10, padx=10)
                                    return
                                else:
        ...
        case "Show expenses(table)":
            ...
            def add_expense(category, amount, date):
                ...
                if date is not None and date != "":
                    if not Finance.validate_date(date):
                        if frame4 is None or not frame4.winfo_exists():
                            create_frame4(1, 2, window)
                    ...
                    if frame4 is None or not frame4.winfo_exists():
                        create_frame4(1, 2, window)
                    error_label = customtkinter.CTkLabel(master=frame4, text="No date entered. Using current date: " + Finance.current_date, text_color="pink", font=("Roboto", 16))
                    error_label.grid(row=1, column=0, pady=10, padx=10)
                ...
                date = Finance.current_date if not date else date
                Finance.add_expense(amount, date, category)
                Finance.log.add_expense(category, amount, date)

                last_savings = float(Finance.find_last_savings_value(date))
                if date == Finance.current_date:
                    Finance.update_savings(last_savings - float(amount), date)
                else:
                    Finance.add_saving(last_savings - float(amount), date)
                    
                Finance.log.update_savings(last_savings - float(amount), last_savings)

                item_id = Finance.all_expense_lines[-1][1]
            ...
            table = ttk.Treeview(frame2, 
                                ...
                                show="headings", 
                                height=len(Finance.all_expense_lines) if len(Finance.all_expense_lines) < 15 else 15,
                                style="Treeview",
                                selectmode='browse')
            ...
            for i, x in enumerate(Finance.all_expense_lines):
            ...
            def delete_expense(event=None):
                ...
                item_id = x[0]
                Finance.delete_expense(item_id)
        ...
        case "Graph expenses":
            ...
            def month_selection(choice):
                if choice == "All months":
                    fig = Finance.monthly_expenses_graph()
                elif choice == "Current month":
                    fig = Finance.expenses_graph(float(Finance.current_month))
                else:
                    fig = Finance.expenses_graph(float(choice))
                canvas = FigureCanvasTkAgg(fig, master=frame2)
                canvas.draw()
                canvas.get_tk_widget().grid(row=0, column=0, sticky="nsew")

            month_selection(Finance.current_month)  # Show current month graph by default

            combobox2 = customtkinter.CTkComboBox(master=frame1, values=["Current month", *Finance.get_all_months(), "All months"], command=month_selection)
            combobox2.grid(row=4, column=0, pady=10, padx=10)
...
window.mainloop()
window.protocol("WM_DELETE_WINDOW", Finance.log.login("logged out"))