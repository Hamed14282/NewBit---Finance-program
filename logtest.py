...
def login(type):
    ...
    match type:
        case "logged in":
            with open(f"data/{user_id}/{float(current_month)}/{float(current_month)}_logs.txt", "a") as log_file:
...
def delete_empty_file(file_name):
    user = get_user()
    user_id = get_user_id()

    with open(f"data/{user_id}/{float(current_month)}/{float(current_month)}_logs.txt", "a") as log_file:
        log_file.write(f"{current_date} - {get_current_time()} |Auto| Deleted {file_name} file: empty\n")
